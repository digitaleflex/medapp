...
// AJOUT IMPORTANT : Inclure l'autoloader de Composer
require_once __DIR__ . '/../vendor/autoload.php';

// Utiliser le namespace Google
...
class GoogleAuth {
    ...
    public function __construct() {
        ...
        // Initialiser le client Google (utiliser GoogleClient au lieu de Google_Client)
    ...
    public function loginOrRegisterUser($user_info) {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            try {
                ...
                // Déterminer le rôle (patient par défaut)
                $role = 'patient';
                
                // Vérifier si l'email correspond à un domaine médical autorisé
                $medical_domains = ['hopital.fr', 'clinique.fr', 'medecin.fr']; // À personnaliser
                $email_domain = substr(strrchr($user_info['email'], "@"), 1);
                if (in_array($email_domain, $medical_domains)) {
                    $role = 'medecin_pending'; // Statut en attente de validation
                }
                
                // Insérer dans la table users
                $stmt = $this->db->prepare("
                    INSERT INTO users (nom, prenom, email, google_id, role, created_at) 
                ...
                // Insérer dans la table correspondante selon le rôle
                if ($role === 'medecin_pending') {
                    $stmt = $this->db->prepare("
                        INSERT INTO medecin (id, nom, prenom, email, status) 
                        VALUES (:id, :nom, :prenom, :email, 'pending')
                    ");
                } else {
                    $stmt = $this->db->prepare("
                        INSERT INTO patient (id, nom, prenom, email) 
                ...
                // Initialiser la session avec statut spécial pour médecin en attente
                if ($role === 'medecin_pending') {
                    $_SESSION['auth_redirect'] = '../views/auth/pending_approval.php';
                    $_SESSION['pending_message'] = 'Votre compte médecin est en attente de validation par l\'administrateur.';
                } else {
                    $_SESSION['auth_redirect'] = '../views/patient/dashboard.php';
                }
                
                initSession($user_id, $role, $user_info['family_name'], $user_info['given_name'], $user_info['email'], 'google');
